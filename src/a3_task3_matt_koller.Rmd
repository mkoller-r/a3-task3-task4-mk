...
ggplot(data = urchin_size, aes(x = size)) + #create a ggplot based on the urchin_size data; the size will go along the x-axis
  geom_histogram(bins = 20) + #the ggplot will be a histogram, with bins equalling 20
...
percentage_difference <- (mean_difference/mean_size_red_urchin)*100 #Calculating the percentage difference according to the following: on average, how much bigger (percentage-wise) are red urchins than purple urchins?
```
...
ggplot(data = purple_urchin_summary, aes(x = year, y = n))+ #this makes a graph comparing the year with the urchin count during each year. The "n" comes from the column generated in the previous code chunk by using the count function to tally the number of urchin observations at each site for each year.
  geom_col(aes(color = site_full)) + #This creates a bar graph demonstrating the counts of urchins by year at each reef. I originally tried doing a histogram and was getting tripped up, because the histogram only requires a single x variable and I couldn't find a way to give it a y variable (or somehow change my code in th eprevious chunk such that it was counting the number of observations), so I checked the Slack and others were having issues too....issues which were solved by using a geom_col!
